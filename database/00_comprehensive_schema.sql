...
BEGIN
    ...
    DELETE FROM task_executions WHERE created_at < NOW() - INTERVAL '1 day' * days_to_keep;
    ...
    DELETE FROM audit_logs WHERE created_at < NOW() - INTERVAL '1 day' * days_to_keep;
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    ...
    result := result || jsonb_build_object('system_metrics_deleted', deleted_count);
...
('Default Organization', '323', '{"default": true}');

-- Insert default admin user
INSERT INTO users (organization_id, email, name, role) VALUES 
((SELECT id FROM organizations WHERE codegen_org_id = '323'), 'admin@example.com', 'System Administrator', 'admin');

-- Insert default project
INSERT INTO projects (organization_id, name, description, status) VALUES 
((SELECT id FROM organizations WHERE codegen_org_id = '323'), 'Default Project', 'Default project for system initialization', 'active');

-- Insert default codegen agent
INSERT INTO codegen_agents (organization_id, name, description, agent_type, configuration) VALUES 
((SELECT id FROM organizations WHERE codegen_org_id = '323'), 'Default Agent', 'Default Codegen agent for task execution', 'general', '{"model": "claude-3-5-sonnet-latest"}');

-- =====================================================
-- COMMENTS FOR DOCUMENTATION
-- =====================================================
COMMENT ON TABLE organizations IS 'Top-level tenant isolation for multi-organization support';
COMMENT ON TABLE projects IS 'Project management with repository and integration tracking';
COMMENT ON TABLE tasks IS 'Task lifecycle management with dependency tracking';
COMMENT ON TABLE pipelines IS 'CI/CD pipeline definitions and execution tracking';
COMMENT ON TABLE codegen_agents IS 'Codegen SDK agent management and configuration';
COMMENT ON TABLE integration_events IS 'Event tracking for GitHub, Linear, and Slack integrations';
COMMENT ON TABLE learning_patterns IS 'Machine learning patterns for system optimization';
COMMENT ON TABLE knowledge_base IS 'Organizational knowledge base for context and learning';
...
DO $$
BEGIN
    RAISE NOTICE 'Comprehensive Graph-Sitter database schema initialized successfully!';
    RAISE NOTICE 'Modules: Core System, Task Management, CI/CD Pipelines, Codegen SDK, Integrations, Analytics, Learning';
