...
class Codebase(
        ...
        *,
        ...
        if self.ctx.config.use_pink != PinkMode.OFF:
            try:
                ...
                import warnings
                warnings.warn(
                    f"Pink SDK is configured but not installed. "
                    f"Install with: pip install codegen-sdk-pink or pip install 'graph-sitter[pink]'",
                    UserWarning,
                    stacklevel=2
                )
                # Fallback to OFF mode if Pink SDK is not available
                self.ctx.config.use_pink = PinkMode.OFF
                self._pink_codebase = None
            except Exception as e:
                import warnings
                warnings.warn(
                    f"Failed to initialize Pink SDK: {e}. Falling back to Python-only parsing.",
                    UserWarning,
                    stacklevel=2
                )
                self.ctx.config.use_pink = PinkMode.OFF
    ...
    def files(self, *, extensions: list[str] | Literal["*"] | None = None) -> list[TSourceFile] | list[File]:
        ...
        """
        if self.ctx.config.use_pink == PinkMode.ALL_FILES and self._pink_codebase is not None:
    ...
    def has_file(self, filepath: str, ignore_case: bool = False) -> bool:
        ...
        if self.ctx.config.use_pink == PinkMode.NON_SOURCE_FILES and self._pink_codebase is not None:
