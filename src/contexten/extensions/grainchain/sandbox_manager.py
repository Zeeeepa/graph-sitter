...
from .types import (
    SandboxProvider, SandboxConfig, SandboxMetrics, 
    IntegrationStatus, GrainchainEvent, GrainchainEventType
)
...
class SandboxManager:
    ...
    async def create_session(
        ...
        async with self.client.create_sandbox(config, provider) as session:
            # Track session
    ...
    async def _select_optimal_provider(self, config: SandboxConfig) -> SandboxProvider:
        ...
        """
        available_providers = await self.client.get_available_providers()
        ...
        provider_metrics = await self.client.get_metrics()
    ...
    async def optimize_resources(self) -> Dict[str, Any]:
        ...
        long_running_sessions = [
            ...
            if (datetime.utcnow() - info['created_at']).total_seconds() > 3600  # 1 hour
        ]
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            availability_ratio = available_count / max(1, total_providers)
            ...
            if availability_ratio < 1.0:
                unavailable = set(self.config.get_enabled_providers()) - set(available_providers)
                issues.append(f"Providers unavailable: {[p.value for p in unavailable]}")
            
            # Check error rates
            for provider, metrics in provider_metrics.items():
                if metrics.success_rate < 0.9:
                    issues.append(f"High error rate for {provider.value}: {(1-metrics.success_rate)*100:.1f}%")
