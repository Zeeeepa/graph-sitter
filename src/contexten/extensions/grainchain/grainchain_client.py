...
from .types import (
    ...
    ProviderStatus, SandboxMetrics, GrainchainEvent, GrainchainEventType
)
...
class GrainchainClient:
    ...
    async def select_provider(
        ...
        if not available_providers:
            raise ProviderUnavailableError("No providers are currently available")
    ...
    async def create_sandbox(
    ...
    ) -> AsyncContextManager['SandboxSession']:
        """
        ...
        if not client:
            raise ProviderUnavailableError(f"Provider {selected_provider.value} not available")
        ...
        try:
            ...
            # Emit event
            await self._emit_event(GrainchainEventType.SANDBOX_CREATED, {
                'sandbox_id': sandbox_id,
            ...
            try:
                ...
                try:
                    ...
                    # Emit event
                    await self._emit_event(GrainchainEventType.SANDBOX_DESTROYED, {
                        'sandbox_id': sandbox_id,
                        'provider': selected_provider.value
                    })
            ...
            logger.error(f"Failed to create sandbox with {selected_provider.value}: {e}")
            raise SandboxCreationError(f"Sandbox creation failed: {e}")
        ...
        for prov in providers_to_check:
            ...
            if client:
                try:
                    ...
                    logger.error(f"Failed to list snapshots for {prov.value}: {e}")
...
class SandboxSession:
    ...
    async def create_snapshot(self, name: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        # Emit event
        await self._grainchain_client._emit_event(GrainchainEventType.SNAPSHOT_CREATED, {
    ...
    async def restore_snapshot(self, snapshot_id: str) -> None:
        ...
        # Emit event
        await self._grainchain_client._emit_event(GrainchainEventType.SNAPSHOT_RESTORED, {
            'sandbox_id': self.sandbox_id,
            'snapshot_id': snapshot_id,
            'provider': self.provider.value
        })
...
class MockProviderClient:
    ...
    async def create_sandbox(self, config: SandboxConfig) -> str:
        return f"sandbox_{self.__class__.__name__}_{datetime.utcnow().timestamp()}"
    
    async def destroy_sandbox(self, sandbox_id: str) -> None:
        pass
    
    async def execute(self, sandbox_id: str, command: str, timeout: Optional[int] = None) -> ExecutionResult:
    ...
    async def create_snapshot(self, sandbox_id: str, name: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        return f"snapshot_{name}_{datetime.utcnow().timestamp()}"
